...
exports.ExcelDataAggregationService = void 0;
...
const getTimeFromReport = (report) => {
...
exports.ExcelDataAggregationService = {
    async aggregateDataForExcel(voyageId) {
        ...
        const allReportsForVoyage = report_model_1.default._getAllReportsForVoyage(voyageId);
        ...
        if (approvedReports.length === 0)
            throw new Error(`No approved reports found for voyage ${voyageId}.`);
        const departureReport = approvedReports.find(r => r.reportType === 'departure');
        const arrivalReport = approvedReports.find(r => r.reportType === 'arrival');
        const noonReports = approvedReports.filter(r => r.reportType === 'noon');
        const arrivalAnchorNoonReports = approvedReports.filter(r => r.reportType === 'arrival_anchor_noon');
        const berthReports = approvedReports.filter(r => r.reportType === 'berth');
        // Pass voyage.id as the currentVoyageId, and voyage.startDate for reference / logging
        const nextVoyageDepartureReport = await voyage_lifecycle_service_1.VoyageLifecycleService.getNextVoyageDepartureReport(vessel.id, voyage.id, voyage.startDate);
        ...
        if (departureReport) {
            ...
            phase1_data.distanceTravelled += departureReport.harbourDistance ?? 0;
        }
        let arrivalAtAnchorageTimeMillis = null;
        let reportAtAnchorage = undefined;
        if (arrivalAnchorNoonReports.length > 0) {
            reportAtAnchorage = arrivalAnchorNoonReports[arrivalAnchorNoonReports.length - 1];
            if (reportAtAnchorage && reportAtAnchorage.noonDate && reportAtAnchorage.noonTime) {
                arrivalAtAnchorageTimeMillis = getTimeFromReport(reportAtAnchorage);
            }
        }
        else if (arrivalReport) {
            reportAtAnchorage = arrivalReport;
            if (arrivalReport.eospDate && arrivalReport.eospTime) {
                arrivalAtAnchorageTimeMillis = getTimeFromReport({ reportType: 'arrival', reportDate: arrivalReport.eospDate, reportTime: arrivalReport.eospTime });
            }
        }
        ...
        if (reportAtAnchorage) {
            if (reportAtAnchorage.reportType === 'arrival') {
                const arrivalData = reportAtAnchorage;
        ...
        if (nextVoyageDepartureReport) {
            ...
            console.log(`[ExcelDataAggregation] Next voyage harbour distance: ${nextVoyageDepartureReport.harbourDistance}`);
            plus1_Hours = nextVoyageDepartureReport.meDailyRunHours ?? 0;
            plus1_Distance = nextVoyageDepartureReport.harbourDistance ?? 0;
        ...
        const cargoUnits = departureReport?.cargoUnits ?? null;
        const cargoDefaultWeight = departureReport?.cargoDefaultWeight ?? null;
        const cargoInCaseApplicable = departureReport?.cargoInCaseApplicable ?? null;
