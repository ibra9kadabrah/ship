...
const getTimeFromReport = (report: Partial<Report>): number => {
    ...
    if (report.reportType === 'noon' || report.reportType === 'arrival_anchor_noon') {
        ...
        dateStr = (report as DepartureSpecificData).reportDate;
        timeStr = (report as DepartureSpecificData).reportTime;
    } else if (report.reportType === 'arrival') {
        dateStr = (report as ArrivalSpecificData).reportDate;
    ...
    if (dateStr && timeStr) {
        try {
            ...
            return new Date(`${dateStr}T${formattedTime}Z`).getTime();
        } catch (e) {
            console.error(`[ExcelDataAggregationService] Error parsing date/time for report ${report.id} during sorting:`, e);
            return 0; 
        }
    }
    return 0; 
...
export const ExcelDataAggregationService = {
    async aggregateDataForExcel(voyageId: string): Promise<AggregatedExcelDataDTO> {
        ...
        const approvedReports = allReportsForVoyage.filter(r => r.status === 'approved').sort((a, b) => getTimeFromReport(a) - getTimeFromReport(b));
        ...
        const nextVoyageDepartureReport = await VoyageLifecycleService.getNextVoyageDepartureReport(vessel.id, voyage.startDate);
        ...
        let comprehensiveTotalHoursUnderway = phase1_data.hoursUnderway + phase2_data.hoursUnderway;
        ...
        const totalHoursUnderwayFormatted = formatHHMM(comprehensiveTotalHoursUnderway);

        const totalDistanceTravelledNm = phase1_data.distanceTravelled + phase2_data.distanceTravelled;

        let averageSpeedKnots = 0;
        if (comprehensiveTotalHoursUnderway > 0 && totalDistanceTravelledNm > 0) {
            averageSpeedKnots = totalDistanceTravelledNm / comprehensiveTotalHoursUnderway;
        }
        ...
        const aggregatedData: AggregatedExcelDataDTO = {
            ...
            totalFuelConsumedHfoVoyage: totalFuelConsumedHfoVoyage,
            totalFuelConsumedLfoVoyage: totalFuelConsumedLfoVoyage,
            totalFuelConsumedMgoVoyage: totalFuelConsumedMgoVoyage,
            totalFuelConsumedLpgPVoyage: totalFuelConsumedLpgPVoyage,
            totalFuelConsumedLpgBVoyage: totalFuelConsumedLpgBVoyage,
            totalFuelConsumedLngVoyage: totalFuelConsumedLngVoyage,
            totalFuelConsumedMethanolVoyage: totalFuelConsumedMethanolVoyage,
            totalFuelConsumedEthanolVoyage: totalFuelConsumedEthanolVoyage,
            totalFuelConsumedOtherVoyage: totalFuelConsumedOtherVoyage,

            totalHoursUnderway: totalHoursUnderwayFormatted,
            totalDistanceTravelledNm: totalDistanceTravelledNm,
